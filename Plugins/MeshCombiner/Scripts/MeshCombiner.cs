...
namespace GrozaGames.MeshCombining
{
    ...
    public class MeshCombiner : MonoBehaviour
    {
        ...
        [SerializeField] private bool _combineOnStart;

        [SerializeField] private bool _includeSkinnedMeshRenderers = true;

        [SerializeField] private bool createMultiMaterialMesh = true,
            combineInactiveChildren = false,
            deactivateCombinedChildren = false,
            deactivateCombinedChildrenMeshRenderers = true,
            generateUVMap = false,
        ...
        {
            get => _includeSkinnedMeshRenderers;
            set => _includeSkinnedMeshRenderers = value;
        }
        ...
        private void CombineMeshesWithSingleMaterial(bool showCreatedMeshInfo)
        {
            ...
            {
                var skinnedMeshRenderers = GetComponentsInChildren<SkinnedMeshRenderer>(combineInactiveChildren);
                var oldSize = combineInstances.Length;
                Array.Resize(ref combineInstances, combineInstances.Length + skinnedMeshRenderers.Length);
                var skinnedMeshRendererIndex = 0;
                for (int i = oldSize; i < combineInstances.Length; i++)
                {
                    combineInstances[i].subMeshIndex = 0;
                    combineInstances[i].mesh = skinnedMeshRenderers[skinnedMeshRendererIndex].sharedMesh;
                    combineInstances[i].transform = skinnedMeshRenderers[skinnedMeshRendererIndex].transform.localToWorldMatrix;
                    verticesLength += combineInstances[i].mesh.vertices.Length;
